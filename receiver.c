...
void *threadFunction(void *args)
{
    ...
    if ((s_tcp = socket(PF_INET, SOCK_DGRAM, 0)) < 0)
    {
    ...
    if ((setsockopt(s_tcp, SOL_SOCKET, SO_BINDTODEVICE, (void *)&ifr, sizeof(ifr))) < 0)
    {
    ...
    if ((setsockopt(s_tcp, IPPROTO_IP, IP_ADD_MEMBERSHIP, (void *)&mcast_req, sizeof(mcast_req))) < 0)
    {
    ...
    while (1)
    {
        if (done == 0)
        {
            ...
            len = recvfrom(s_tcp, buf, sizeof(buf), 0, (struct sockaddr *)&mcast_saddr, &mcast_saddr_len);
